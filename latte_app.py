...
def load_all():
    ...
    # 기존 DB 삭제 후 새로 빌드
    shutil.rmtree('chroma_syllabus_db', ignore_errors=True)
    shutil.rmtree('chroma_review_db', ignore_errors=True)
    print('기존 DB 삭제 완료 — OpenAI 임베딩으로 새로 빌드합니다.')


    class EmbeddingsWithRateLimit:
        BATCH_SIZE = 100
        BATCH_DELAY = 0.5
        RETRY_WAIT = 30
        MAX_RETRIES = 5
        ...
        def __init__(self, model='text-embedding-3-small'):
            self._base = OpenAIEmbeddings(model=model)
            ...
            for attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    if '429' in msg or 'rate' in msg.lower() or 'quota' in msg.lower():
                        ...
                        print(f'  ⚠️  Rate limit — {wait}초 대기 후 재시도 ({attempt + 1}/{self.MAX_RETRIES})...')
                        time.sleep(wait)
                    else:
                        raise
            raise RuntimeError(f'임베딩 실패: {self.MAX_RETRIES}회 재시도 후에도 오류')
        ...
        def embed_documents(self, texts):
            ...
            for start in range(0, total, self.BATCH_SIZE):
                ...
                print(f'  임베딩 중... {end}/{total}', end='\r')
                results.extend(self._embed_batch(batch))
                if end < total:
                    time.sleep(self.BATCH_DELAY)
            print(f'  임베딩 완료: {total}개')
            return results
    ...
    embeddings = EmbeddingsWithRateLimit()
        ...
        with st.spinner('☕ 선배들의 후기를 찾아보는 중...'):
            try:
                ...
                if any(kw in user_input.lower() for kw in review_keywords):
                    full_answer = '📝 후기 입력 기능은 현재 준비 중이에요! 노트북 버전에서 이용해주세요.'
